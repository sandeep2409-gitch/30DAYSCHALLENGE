...
class EmotionModelManager:
    ...
    def load_model_thread(self, model_name):
        try:
            ...
            try:
                print(f"[ModelManager] Attempting to load '{model_name}' from local cache...")
                classifier = pipeline(
                    "text-classification",
                    model=model_name,
                    top_k=None,
                ...
                print(f"[ModelManager] Local cache load failed: {cache_err}. Downloading from Hugging Face Hub on device '{self.device}'...")
                classifier = pipeline(
                    "text-classification",
                    model=model_name,
                    top_k=None,
